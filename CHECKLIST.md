...
### Native push (the actual iOS/Android app, not the browser)

The Web Push pipeline above only works in a real mobile *browser* tab — it
does not work inside the Capacitor-wrapped app itself. That needed a
completely separate mechanism (real APNs/FCM), which is now built:

- [x] `@capacitor/push-notifications` installed; `npm run cap:sync` picks it
      up on both platforms
- [x] Android's `POST_NOTIFICATIONS` permission added to
      `AndroidManifest.xml` (required at runtime on API 33+, same idea as
      camera/location)
- [x] `src/lib/nativePush.js` — requests permission, calls
      `PushNotifications.register()`, and saves the resulting device token
      to a new `native_push_tokens` table. `PushToggle.jsx` now branches on
      `Capacitor.isNativePlatform()`: same "🔔 Push notifications" card
      either way, native devices go through this new path, browsers keep
      using the old Web Push path untouched (checked via a live dynamic
      import in the browser — `isNativePushSupported()` still reports
      `false` there, zero behavior change for web users)
- [x] `supabase/migration_native_push_tokens.sql` — new table, one row per
      device token, RLS scoped to `auth.uid()` same as every other
      user-owned table here
- [x] `send-push` edge function rewritten to deliver to both tables in
      parallel: Web Push to `push_subscriptions` (unchanged), and FCM's v1
      HTTP API to `native_push_tokens` (the *only* current option — Google
      shut down the old legacy server-key API in June 2024). It signs its
      own OAuth JWT from a service account key using Deno's built-in Web
      Crypto, so no extra npm dependency was needed
- [x] iOS "Push Notifications" capability wired directly into the project
      files (same effect as the Xcode UI button): `App.entitlements` with
      `aps-environment=development` (Xcode/App Store Connect rewrites this
      to `production` automatically at archive/export time against a real
      Distribution profile — leave it as `development` in the source file),
      wired into both build configs via `CODE_SIGN_ENTITLEMENTS`, plus
      `UIBackgroundModes: remote-notification` in `Info.plist` so a
      background push can actually arrive, not just a foreground one
- [x] Android's Gradle side needed **no manual edits at all** — the
      Capacitor-generated `android/app/build.gradle` already only applies
      the Google Services plugin *if* `google-services.json` exists
      (`try { apply plugin: ... } catch { /* skipped */ }`), and
      `android/build.gradle` already has the `com.google.gms:google-services`
      classpath from `npx cap sync` adding the plugin. Just drop the file
      in and it activates itself
- [x] Verified all of the above compiles clean with zero Firebase/APNs
      credentials in place yet: `./gradlew assembleDebug` and `xcodebuild`
      both succeed, and a simulator launch shows no visual/functional
      regression. This is expected and by design — the native send path is
      a silent no-op until the secret below is set, so none of this is
      "half-broken," it's just inert until configured

**What's left is entirely account-gated — a Firebase project and an Apple
Developer enrollment, both of which have to be created by you, not me:**

- [ ] **Apple Developer Program enrollment** ($99/yr, developer.apple.com)
      — same one already listed under "Mobile app store setup" below; push
      needs it too, since a real APNs key can only be generated by an
      enrolled account
- [ ] **Create a Firebase project** (console.firebase.google.com, free) —
      one project covers both platforms; Firebase forwards iOS deliveries
      to APNs internally, so you don't integrate APNs directly
- [ ] In that project, **add an Android app** with package name
      `com.yourplatesapp.app` (or whatever the real final package name ends
      up being — see the bundle-ID placeholder note below) and download the
      generated **`google-services.json`** into `android/app/` (same folder
      as `android/app/build.gradle`)
- [ ] In the same project, **add an iOS app** with the same bundle ID and
      download **`GoogleService-Info.plist`** into `ios/App/App/` — then in
      Xcode, drag it into the App target if it doesn't show up in the file
      list automatically (Add Files to "App", make sure "Copy items if
      needed" and the App target are both checked)
- [ ] In Apple Developer → Certificates, Identifiers & Profiles → Keys,
      **create an APNs Auth Key** (.p8, one-time, works for every app under
      your account) and download it — Apple only lets you download it
      once, so keep it somewhere safe
- [ ] In Firebase Console → Project Settings → Cloud Messaging → Apple app
      configuration, **upload that .p8 key** along with its Key ID and your
      Apple Team ID — this is the step that actually lets Firebase deliver
      to iOS devices
- [ ] In Firebase Console → Project Settings → Service Accounts →
      **Generate new private key** — downloads a JSON file. Set its entire
      contents as a Supabase Edge Function secret:
      ```bash
      npx supabase secrets set FCM_SERVICE_ACCOUNT_JSON="$(cat path/to/downloaded-key.json)"
      ```
- [ ] Redeploy the function so it picks up the secret:
      `npx supabase functions deploy send-push --no-verify-jwt`
- [ ] Run `supabase/migration_native_push_tokens.sql` in the SQL Editor
      (same paste-and-run process as every other migration file here)
- [ ] `npm run cap:sync`, rebuild both native apps, and test on a **real
      device** — Apple does not deliver actual network APNs pushes to the
      iOS Simulator (only Xcode's separate simulated-push tool can, which
...
- [x] Native push (real APNs/FCM, not just the browser's Web Push) is now
      built — client plugin, DB table, edge function, and the iOS Push
      Notifications capability are all wired and verified compiling clean
      on both platforms. What's left is entirely account-gated (a Firebase
      project + Apple Developer enrollment, both need your own identity) —
      full step-by-step list in "Push notifications setup" → "Native push"
      above
